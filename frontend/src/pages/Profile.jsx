...
import {
  faUser, faEdit, faSave, faTimes, faBriefcase, faTrophy, faCheckCircle, faClock, faMagicWandSparkles
...
const Profile = () => {
  ...
  const handleGenerateBio = async () => {
    try {
      showToast("✨ Generating bio...", "success");
      const response = await axiosInstance.post("/api/profiles/generate-bio/");
      if (response.data.bio) {
        setEditForm(prev => ({ ...prev, bio: response.data.bio }));
      ...
      showToast("Error generating bio", "error");
    }
  };
  ...
  const styles = {
    ...
    input: { width:'100%', padding:'12px', borderRadius:'8px', border:'1px solid #d1d5db', marginBottom:'12px', fontSize:'16px' },
    textarea: { width:'100%', padding:'12px', borderRadius:'8px', border:'1px solid #d1d5db', minHeight:'100px', resize:'vertical', fontSize:'16px', marginBottom:'12px' },
    ...
    <div style={styles.wrapper}>
        ...
        <div style={styles.grid}>
          ...
          <div style={styles.main}>
            <div style={styles.section}>
              ...
              {editing ? (
                <>
                  {["email","username","school","specialization","batch_year","role","linkedin_url","github_url","portfolio_url"].map(f => (
                    <input
                      key={f}
                      style={styles.input}
                      value={editForm[f] || ''}
                  ...
                  <div>
                    ...
                    />
                    <button
                      style={{ ...styles.button, ...styles.saveButton, marginTop:"8px", background:"#9333EA" }}
                      type="button"
                      onClick={handleGenerateBio}
                    >
                      <FontAwesomeIcon icon={faMagicWandSparkles} /> Generate Bio
                    </button>
                  </div>
                </>
              ) : (
      ...
      {toast.show && (
        <motion.div
          ...
          style={{...styles.toast, background: toast.type==='success' ? '#10B981' : '#EF4444'}}
        >
          {toast.message}
        </motion.div>
      )}
